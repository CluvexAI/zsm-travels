...
import { Search, X, Check, ShieldCheck, AlertCircle, Grid, Shield } from 'lucide-react';

const mockReservations = [
  { id: 'ZSM-10041', cxId: 'CX-1001', pnr: 'XKRT4P', pnrStatus: 'PNR Created', firstName: 'John', lastName: 'Smith', email: 'john.smith@email.com', phone: '+1 212-555-0141', altPhone: '+1 212-555-0199', route: 'JFK → LAX', origin: 'JFK', destination: 'LAX', airline: 'American Airlines', flightNumber: 'AA 100', travelDate: '2026-08-15', bookingDate: '2026-07-18', status: 'Confirmed', agent: 'Agent A', insuranceStatus: 'No Insurance Added', 
    passengers: [{ id: 1, name: 'John Smith', type: 'Adult', insurance: [] }] 
  },
  { id: 'ZSM-10042', cxId: 'CX-1002', pnr: 'BMNW2L', pnrStatus: 'PNR Created', firstName: 'Emily', lastName: 'Johnson', email: 'emily.johnson@email.com', phone: '+1 312-555-0198', altPhone: '', route: 'ORD → MIA', origin: 'ORD', destination: 'MIA', airline: 'Delta Air Lines', flightNumber: 'DL 455', travelDate: '2026-08-20', bookingDate: '2026-07-19', status: 'Confirmed', agent: 'Agent B', insuranceStatus: 'Insurance Added', 
    passengers: [{ id: 1, name: 'Emily Johnson', type: 'Adult', insurance: [{ id: 'ins_1', label: 'Comprehensive Protection', price: 50, type: 'Comprehensive', status: 'Added' }] }, { id: 2, name: 'Robert Johnson', type: 'Adult', insurance: [] }] 
  },
  { id: 'ZSM-10043', cxId: 'CX-1003', pnr: '', pnrStatus: 'Not Created', firstName: 'Robert', lastName: 'Williams', email: 'robert.williams@email.com', phone: '+1 415-555-0176', altPhone: '+1 415-555-0188', route: 'SFO → SEA', origin: 'SFO', destination: 'SEA', airline: 'United Airlines', flightNumber: 'UA 12', travelDate: '2026-08-10', bookingDate: '2026-07-17', status: 'On Hold', agent: 'Agent A', insuranceStatus: 'No Insurance Added', 
    passengers: [{ id: 1, name: 'Robert Williams', type: 'Adult', insurance: [] }] 
  }
];

const insuranceOptions = [
  { id: 'bsc_1', type: 'Basic', label: 'Basic Medical Cover', price: 25 },
  { id: 'bsc_2', type: 'Basic', label: 'Basic Trip Cancellation', price: 35 },
  { id: 'comp_1', type: 'Comprehensive', label: 'Comprehensive Protection', price: 50 },
  { id: 'comp_2', type: 'Comprehensive', label: 'Premium World Cover', price: 120 },
  { id: 'med_1', type: 'Medical', label: 'Emergency Evacuation', price: 40 },
];
...
const AddInsurance = () => {
  const [searchFilters, setSearchFilters] = useState({
    ...
    fromDate: '', toDate: '', insuranceStatus: 'All Statuses'
  });
  
  const [searchResults, setSearchResults] = useState([]);
  const [hasSearched, setHasSearched] = useState(false);
  const [selectedBooking, setSelectedBooking] = useState(null);
  const [isInsurancePanelOpen, setIsInsurancePanelOpen] = useState(false);
  
  const [selectedPassenger, setSelectedPassenger] = useState(null);
  const [selectedInsuranceType, setSelectedInsuranceType] = useState('Basic');
  ...
  const handleSearch = (e) => {
    ...
    const results = mockReservationsState.filter(r => {
      ...
      if (searchFilters.insuranceStatus !== 'All Statuses' && r.insuranceStatus !== searchFilters.insuranceStatus) match = false;
  ...
  const openInsurancePanel = (booking) => {
    ...
    setIsInsurancePanelOpen(true);
  };

  const closeInsurancePanel = () => {
    setIsInsurancePanelOpen(false);
    setSelectedBooking(null);
  };

  const confirmInsuranceAssignment = () => {
    if (!selectedInsuranceOption) return;
    if (agentNote.trim() === '') {
  ...
  const processPaymentAndAdd = () => {
    const finalPrice = customPrice !== '' ? parseFloat(customPrice) : selectedInsuranceOption.price;
    
    const updatedPassengers = selectedBooking.passengers.map(p => {
       if (p.id === selectedPassenger.id) {
           const passengerInsurance = [...p.insurance];
           passengerInsurance.push({
             id: Math.random().toString(36).substring(2, 9),
             label: selectedInsuranceOption.label,
             price: finalPrice,
             payment: paymentMethod,
             type: selectedInsuranceOption.type,
             status: 'Added',
             note: agentNote.trim() !== '' ? { text: agentNote.trim(), timestamp: noteTimestamp } : null
           });
           
           return { ...p, insurance: passengerInsurance };
       }
       return p;
    });

    let hasAnyInsurance = false;
    updatedPassengers.forEach(p => {
      if (p.insurance.length > 0) hasAnyInsurance = true;
    });

    const newInsuranceStatus = hasAnyInsurance ? 'Insurance Added' : 'No Insurance Added';
    const updatedBooking = { ...selectedBooking, passengers: updatedPassengers, insuranceStatus: newInsuranceStatus };
  ...
  const handleRemoveInsurance = (insId) => {
     setConfirmAction({
       message: "Are you sure you want to remove this insurance?",
       onConfirm: () => {
         const updatedPassengers = selectedBooking.passengers.map(p => {
           if (p.id === selectedPassenger.id) {
               const passengerInsurance = p.insurance.filter(i => i.id !== insId);
               return { ...p, insurance: passengerInsurance };
           }
           return p;
        });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="page-container" style={{ padding: '2rem' }}>
        <div style={{ marginBottom: '2.5rem' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--text-primary)' }}>Add / Edit Insurance</h1>
          <p style={{ fontSize: '1rem', color: 'var(--text-secondary)' }}>Manage trip protection and medical cover per passenger.</p>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '6px', border: '1px solid #e5e7eb', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          <div style={{ borderBottom: '3px solid #38b2ac', padding: '1.25rem 1.5rem 1rem' }}>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 700, color: '#319795', margin: 0 }}>Insurance Management</h2>
          </div>
          ...
          <div style={{ padding: '1.5rem', background: '#f8fafc' }}>
            ...
            <div style={{ borderTop: '1px solid #e5e7eb', margin: '1.5rem 0 0', paddingTop: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem', maxWidth: '100%' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#6b7280', marginBottom: '0.375rem' }}>INSURANCE STATUS</label>
                  <select value={searchFilters.insuranceStatus} onChange={(e) => handleFilterChange('insuranceStatus', e.target.value)} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.875rem', color: '#374151', backgroundColor: 'white', outline: 'none', cursor: 'pointer' }}>
                    <option value="All Statuses">All Statuses</option>
                    <option value="Insurance Added">Insurance Added</option>
                    <option value="No Insurance Added">No Insurance Added</option>
                  </select>
                </div>
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                   <button type="button" onClick={handleSearch} style={{ width: '100%', padding: '0.6rem', backgroundColor: '#38b2ac', color: 'white', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', textTransform: 'uppercase', transition: 'background-color 0.15s ease' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#319795'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#38b2ac'}>SEARCH</button>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ background: 'var(--bg-base)', color: 'var(--text-secondary)', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                  <tr>
                    ...
                    <th style={{ padding: '1rem 1.5rem', borderBottom: '2px solid var(--border-color)', color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.875rem', textTransform: 'uppercase' }}>Insurance Status</th>
                ...
                <tbody>
                  {searchResults.map((reservation) => (
                    <tr key={reservation.id} style={{ borderTop: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '1rem 1.5rem' }}>
                        <span style={{ padding: '0.25rem 0.75rem', background: reservation.insuranceStatus === 'Insurance Added' ? '#d1fae5' : '#fee2e2', color: reservation.insuranceStatus === 'Insurance Added' ? '#065f46' : '#991b1b', borderRadius: 'var(--radius-full)', fontSize: '0.75rem', fontWeight: 600 }}>
                          {reservation.insuranceStatus}
                        </span>
                      </td>
                      <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                        <button onClick={() => openInsurancePanel(reservation)} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'white', border: '1px solid var(--primary-accent)', color: 'var(--primary-accent)', borderRadius: 'var(--radius-md)', fontWeight: 600, cursor: 'pointer' }}>
                          <ShieldCheck size={16} /> Manage Insurance
                        </button>
                      </td>
                    </tr>
                  ))}
      ...
      {isInsurancePanelOpen && selectedBooking && (
            ...
            <div style={{ padding: '1.5rem 2rem', background: 'var(--bg-topnav)', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               <div>
                  <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                     <Shield size={24} /> Insurance Management
                  </h2>
               ...
               <button onClick={closeInsurancePanel} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                 <X size={24} />
               </button>
            </div>
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ width: '380px', background: '#f8fafc', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                 <div style={{ padding: '1.5rem', background: '#f1f5f9', borderBottom: '1px solid var(--border-color)' }}>
                    <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)' }}>Insurance Summary</h3>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                       {selectedBooking.passengers.map(p => (
                          <button 
                             key={p.id} 
                             onClick={() => { setSelectedPassenger(p); setSelectedInsuranceOption(null); }}
                             style={{ 
                                padding: '1rem', 
                          ...
                          >
                             <div style={{ fontWeight: 600, color: 'var(--text-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                               {p.name}
                               <span style={{ fontSize: '0.75rem', background: p.insurance.length > 0 ? '#d1fae5' : '#e2e8f0', color: p.insurance.length > 0 ? '#065f46' : '#475569', padding: '0.2rem 0.5rem', borderRadius: '99px' }}>
                                 {p.insurance.length > 0 ? 'Covered' : 'Not Covered'}
                               </span>
                             </div>
                          </button>
                       ))}
                    ...
                    {selectedPassenger && (
                      <div style={{ background: 'white', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
                        <div style={{ padding: '1rem' }}>
                          {selectedPassenger.insurance.length === 0 ? (
                            <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>No insurance assigned.</div>
                          ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                              {selectedPassenger.insurance.map(b => (
                                  <div key={b.id} style={{ display: 'flex', flexDirection: 'column' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.875rem' }}>
                                      ...
                                      <button onClick={() => handleRemoveInsurance(b.id)} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem', color: 'var(--danger)', background: 'transparent', border: '1px solid var(--danger)', borderRadius: '4px', cursor: 'pointer' }}>
                                        Remove
                                      </button>
                                    </div>
                                    {b.note && (
              ...
              <div style={{ flex: 1, padding: '2.5rem', display: 'flex', flexDirection: 'column', background: 'white', overflowY: 'auto' }}>
                 <div style={{ marginBottom: '2rem' }}>
                   <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                     Add or Modify Insurance
                   </h3>
                   <p style={{ color: 'var(--text-secondary)' }}>Follow the steps below to add insurance packages for <strong>{selectedPassenger?.name}</strong>.</p>
                 </div>
                 ...
                 <div style={{ marginBottom: '2rem' }}>
                   ...
                   <div style={{ display: 'flex', gap: '1rem' }}>
                     {['Basic', 'Comprehensive', 'Medical'].map(type => (
                       <button
                         key={type}
                         onClick={() => { setSelectedInsuranceType(type); setSelectedInsuranceOption(null); setCustomPrice(''); }}
                         style={{
                           padding: '0.75rem 1.5rem',
                           border: `2px solid ${selectedInsuranceType === type ? 'var(--primary-accent)' : 'var(--border-color)'}`,
                           borderRadius: 'var(--radius-full)',
                           background: selectedInsuranceType === type ? 'var(--primary-accent)' : 'white',
                           color: selectedInsuranceType === type ? 'white' : 'var(--text-primary)',
                           cursor: 'pointer',
                           fontWeight: 600,
                           transition: 'all 0.2s'
                         }}
                 ...
                 <div style={{ marginBottom: '2rem', flex: 1 }}>
                   ...
                   <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                      {insuranceOptions.filter(opt => opt.type === selectedInsuranceType).map(option => (
                        <div 
                          key={option.id}
                          onClick={() => { setSelectedInsuranceOption(option); setCustomPrice(''); }}
                          style={{
                            padding: '1.5rem',
                            border: `2px solid ${selectedInsuranceOption?.id === option.id ? 'var(--primary-accent)' : 'var(--border-color)'}`,
                            borderRadius: 'var(--radius-lg)',
                            cursor: 'pointer',
                            transition: 'all 0.2s',
                            background: selectedInsuranceOption?.id === option.id ? '#f0f9ff' : 'white',
                            boxShadow: selectedInsuranceOption?.id === option.id ? 'var(--shadow-md)' : 'var(--shadow-sm)',
                      ...
                      {insuranceOptions.filter(opt => opt.type === selectedInsuranceType).length === 0 && (
                 ...
                 {selectedInsuranceOption && (
                   <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid #cbd5e1', marginTop: 'auto' }}>
                     ...
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', fontSize: '0.875rem' }}>
                       <span>Base Insurance Premium</span>
                       <span>${customPrice !== '' ? parseFloat(customPrice || 0).toFixed(2) : selectedInsuranceOption.price.toFixed(2)}</span>
                     </div>
                     ...
                     {(() => {
                       const base = customPrice !== '' ? parseFloat(customPrice) || 0 : selectedInsuranceOption.price;
      ...
      {isPaymentModalOpen && selectedInsuranceOption && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'white', padding: '2.5rem', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '400px', boxShadow: 'var(--shadow-xl)', animation: 'slideUp 0.3s ease-out' }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)' }}>Review and confirm the insurance charge.</p>
            </div>
            ...
            <div style={{ background: '#f8fafc', padding: '1rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Insurance Package</span>
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedInsuranceOption.label}</span>
              </div>
