...
import { Search, X, Check, Dog, AlertCircle, Grid } from 'lucide-react';

const mockReservations = [
  { id: 'ZSM-10041', cxId: 'CX-1001', pnr: 'XKRT4P', pnrStatus: 'PNR Created', firstName: 'John', lastName: 'Smith', email: 'john.smith@email.com', phone: '+1 212-555-0141', altPhone: '+1 212-555-0199', route: 'JFK → LAX', origin: 'JFK', destination: 'LAX', airline: 'American Airlines', flightNumber: 'AA 100', travelDate: '2026-08-15', bookingDate: '2026-07-18', status: 'Confirmed', agent: 'Agent A', petStatus: 'No Pet Added', 
    passengers: [{ id: 1, name: 'John Smith', type: 'Adult', pets: [] }] 
  },
  { id: 'ZSM-10042', cxId: 'CX-1002', pnr: 'BMNW2L', pnrStatus: 'PNR Created', firstName: 'Emily', lastName: 'Johnson', email: 'emily.johnson@email.com', phone: '+1 312-555-0198', altPhone: '', route: 'ORD → MIA', origin: 'ORD', destination: 'MIA', airline: 'Delta Air Lines', flightNumber: 'DL 455', travelDate: '2026-08-20', bookingDate: '2026-07-19', status: 'Confirmed', agent: 'Agent B', petStatus: 'Pet Added', 
    passengers: [{ id: 1, name: 'Emily Johnson', type: 'Adult', pets: [{ id: 'pet_1', name: 'Bella', type: 'Dog', label: 'In-Cabin Pet', price: 125, status: 'Added', note: { text: 'Customer travelling with small poodle.', timestamp: '7/24/2026, 10:00:00 AM' } }] }, { id: 2, name: 'Robert Johnson', type: 'Adult', pets: [] }] 
  },
  { id: 'ZSM-10043', cxId: 'CX-1003', pnr: '', pnrStatus: 'Not Created', firstName: 'Robert', lastName: 'Williams', email: 'robert.williams@email.com', phone: '+1 415-555-0176', altPhone: '+1 415-555-0188', route: 'SFO → SEA', origin: 'SFO', destination: 'SEA', airline: 'United Airlines', flightNumber: 'UA 12', travelDate: '2026-08-10', bookingDate: '2026-07-17', status: 'On Hold', agent: 'Agent A', petStatus: 'No Pet Added', 
    passengers: [{ id: 1, name: 'Robert Williams', type: 'Adult', pets: [] }] 
  }
];

const petOptions = [
  { id: 'incabin_1', label: 'In-Cabin Pet', price: 125 },
  { id: 'checked_1', label: 'Checked Pet (Baggage Hold)', price: 200 },
  { id: 'cargo_1', label: 'Cargo Pet Transport', price: 350 },
];
...
const PetBooking = () => {
  const [searchFilters, setSearchFilters] = useState({
    ...
    fromDate: '', toDate: '', petStatus: 'All Statuses'
  });
  ...
  const [inputPetType, setInputPetType] = useState('Dog');
  ...
  const handleSearch = (e) => {
    ...
    const results = mockReservationsState.filter(r => {
      ...
      if (searchFilters.petStatus !== 'All Statuses' && r.petStatus !== searchFilters.petStatus) match = false;
  ...
  const closePetPanel = () => {
    setIsPetPanelOpen(false);
    setSelectedBooking(null);
  };

  const confirmPetAssignment = () => {
    if (!inputPetName.trim()) {
      setAlertMessage("Please enter the Pet's Name.");
      return;
    }
    if (!selectedPetOption) return;
    if (agentNote.trim() === '') {
  ...
  const processPaymentAndAdd = () => {
    const finalPrice = customPrice !== '' ? parseFloat(customPrice) : selectedPetOption.price;
    
    const updatedPassengers = selectedBooking.passengers.map(p => {
       if (p.id === selectedPassenger.id) {
           const passengerPets = [...p.pets];
           passengerPets.push({
             id: Math.random().toString(36).substring(2, 9),
             name: inputPetName.trim(),
             type: inputPetType,
             label: selectedPetOption.label,
             price: finalPrice,
           ...
           return { ...p, pets: passengerPets };
       }
       return p;
    });

    let hasAnyPets = false;
    updatedPassengers.forEach(p => {
      if (p.pets.length > 0) hasAnyPets = true;
    });

    const newPetStatus = hasAnyPets ? 'Pet Added' : 'No Pet Added';
    const updatedBooking = { ...selectedBooking, passengers: updatedPassengers, petStatus: newPetStatus };
  ...
  const handleRemovePet = (petId) => {
     setConfirmAction({
       ...
       onConfirm: () => {
         const updatedPassengers = selectedBooking.passengers.map(p => {
           if (p.id === selectedPassenger.id) {
               const passengerPets = p.pets.filter(pt => pt.id !== petId);
               return { ...p, pets: passengerPets };
           }
           return p;
        });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="page-container" style={{ padding: '2rem' }}>
        <div style={{ marginBottom: '2.5rem' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--text-primary)' }}>Pet Booking</h1>
          <p style={{ fontSize: '1rem', color: 'var(--text-secondary)' }}>Manage pet transportation requests and cabin assignments.</p>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '6px', border: '1px solid #e5e7eb', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          <div style={{ borderBottom: '3px solid #38b2ac', padding: '1.25rem 1.5rem 1rem' }}>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 700, color: '#319795', margin: 0 }}>Pet Management</h2>
          </div>
          ...
          <div style={{ padding: '1.5rem', background: '#f8fafc' }}>
            ...
            <div style={{ borderTop: '1px solid #e5e7eb', margin: '1.5rem 0 0', paddingTop: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem', maxWidth: '100%' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#6b7280', marginBottom: '0.375rem' }}>PET STATUS</label>
                  <select value={searchFilters.petStatus} onChange={(e) => handleFilterChange('petStatus', e.target.value)} style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.875rem', color: '#374151', backgroundColor: 'white', outline: 'none', cursor: 'pointer' }}>
                    <option value="All Statuses">All Statuses</option>
                    <option value="Pet Added">Pet Added</option>
                    <option value="No Pet Added">No Pet Added</option>
                  </select>
                </div>
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                   <button type="button" onClick={handleSearch} style={{ width: '100%', padding: '0.6rem', backgroundColor: '#38b2ac', color: 'white', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', textTransform: 'uppercase', transition: 'background-color 0.15s ease' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#319795'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#38b2ac'}>SEARCH</button>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ background: 'var(--bg-base)', color: 'var(--text-secondary)', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                  <tr>
                    ...
                    <th style={{ padding: '1rem 1.5rem', borderBottom: '2px solid var(--border-color)', color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.875rem', textTransform: 'uppercase' }}>Pet Status</th>
                ...
                <tbody>
                  {searchResults.map((reservation) => (
                    <tr key={reservation.id} style={{ borderTop: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '1rem 1.5rem' }}>
                        <span style={{ padding: '0.25rem 0.75rem', background: reservation.petStatus === 'Pet Added' ? '#d1fae5' : '#fee2e2', color: reservation.petStatus === 'Pet Added' ? '#065f46' : '#991b1b', borderRadius: 'var(--radius-full)', fontSize: '0.75rem', fontWeight: 600 }}>
                          {reservation.petStatus}
                        </span>
                      </td>
                      <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                        <button onClick={() => openPetPanel(reservation)} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'white', border: '1px solid var(--primary-accent)', color: 'var(--primary-accent)', borderRadius: 'var(--radius-md)', fontWeight: 600, cursor: 'pointer' }}>
                          <Dog size={16} /> Manage Pets
                        </button>
                      </td>
                    </tr>
                  ))}
      ...
      {isPetPanelOpen && selectedBooking && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '2rem' }}>
          <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: '1300px', height: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'var(--shadow-xl)' }}>
            ...
            <div style={{ padding: '1.5rem 2rem', background: 'var(--bg-topnav)', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               ...
               <button onClick={closePetPanel} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                 <X size={24} />
               </button>
            </div>
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ width: '380px', background: '#f8fafc', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                 <div style={{ padding: '1.5rem', background: '#f1f5f9', borderBottom: '1px solid var(--border-color)' }}>
                    <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)' }}>Pet Summary</h3>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                       {selectedBooking.passengers.map(p => (
                          <button 
                             key={p.id} 
                             onClick={() => { setSelectedPassenger(p); setSelectedPetOption(null); }}
                             style={{ 
                                padding: '1rem', 
                          ...
                          >
                             <div style={{ fontWeight: 600, color: 'var(--text-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                               {p.name}
                               <span style={{ fontSize: '0.75rem', background: p.pets.length > 0 ? '#d1fae5' : '#e2e8f0', color: p.pets.length > 0 ? '#065f46' : '#475569', padding: '0.2rem 0.5rem', borderRadius: '99px' }}>
                    ...
                    {selectedPassenger && (
                      <div style={{ background: 'white', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
                        <div style={{ padding: '1rem' }}>
                          {selectedPassenger.pets.length === 0 ? (
                            <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>No pets added for this passenger.</div>
                          ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                              {selectedPassenger.pets.map(pt => (
                                  <div key={pt.id} style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem', marginBottom: '0.75rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.875rem' }}>
                                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{pt.name} ({pt.type})</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{pt.label} • {pt.status}</span>
                                      </div>
                                      <button onClick={() => handleRemovePet(pt.id)} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem', color: 'var(--danger)', background: 'transparent', border: '1px solid var(--danger)', borderRadius: '4px', cursor: 'pointer' }}>
                                        Remove
                                      </button>
                                    </div>
                                    {pt.note && (
                                      <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f1f5f9', borderRadius: '4px', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                                        <strong>Note ({pt.note.timestamp}):</strong> {pt.note.text}
                                      </div>
                                    )}
                                  </div>
                              ))}
              ...
              <div style={{ flex: 1, padding: '2.5rem', display: 'flex', flexDirection: 'column', background: 'white', overflowY: 'auto' }}>
                 <div style={{ marginBottom: '2rem' }}>
                   ...
                   <p style={{ color: 'var(--text-secondary)' }}>Record pet details and add transport options for <strong>{selectedPassenger?.name}</strong>.</p>
                 </div>
                 ...
                 <div style={{ marginBottom: '2rem' }}>
                   ...
                   <div style={{ display: 'flex', gap: '1rem' }}>
                     <div style={{ flex: 1 }}>
                       <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Pet Name</label>
                       <input 
                         type="text" 
                         value={inputPetName} 
                         onChange={(e) => setInputPetName(e.target.value)} 
                         placeholder="e.g. Bella" 
                         style={{ width: '100%', padding: '0.75rem', border: '1px solid #cbd5e1', borderRadius: '4px', outline: 'none', boxSizing: 'border-box' }}
                       />
                     </div>
                     <div style={{ flex: 1 }}>
                       <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Pet Type</label>
                       <select 
                         value={inputPetType} 
                         onChange={(e) => setInputPetType(e.target.value)}
                         style={{ width: '100%', padding: '0.75rem', border: '1px solid #cbd5e1', borderRadius: '4px', outline: 'none', boxSizing: 'border-box' }}
                       >
                 ...
                 <div style={{ marginBottom: '2rem', flex: 1 }}>
                   ...
                   <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                      {petOptions.map(option => (
                        <div 
                          key={option.id}
                          onClick={() => { setSelectedPetOption(option); setCustomPrice(''); }}
                          style={{
                            padding: '1.5rem',
                            border: `2px solid ${selectedPetOption?.id === option.id ? 'var(--primary-accent)' : 'var(--border-color)'}`,
                            borderRadius: 'var(--radius-lg)',
                            cursor: 'pointer',
                            transition: 'all 0.2s',
                            background: selectedPetOption?.id === option.id ? '#f0f9ff' : 'white',
                            boxShadow: selectedPetOption?.id === option.id ? 'var(--shadow-md)' : 'var(--shadow-sm)',
                        ...
                        >
                          ...
                          <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--primary-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            $
                            {selectedPetOption?.id === option.id ? (
                               <input 
                                 type="number"
                 ...
                 {selectedPetOption && (
                   <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid #cbd5e1', marginTop: 'auto' }}>
                     ...
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', fontSize: '0.875rem' }}>
                       <span>Base Pet Fee</span>
                       <span>${customPrice !== '' ? parseFloat(customPrice || 0).toFixed(2) : selectedPetOption.price.toFixed(2)}</span>
                     </div>
                     ...
                     {(() => {
                       const base = customPrice !== '' ? parseFloat(customPrice) || 0 : selectedPetOption.price;
      ...
      {isPaymentModalOpen && selectedPetOption && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'white', padding: '2.5rem', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '400px', boxShadow: 'var(--shadow-xl)', animation: 'slideUp 0.3s ease-out' }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)' }}>Review and confirm the pet booking charge.</p>
            </div>
            ...
            <div style={{ background: '#f8fafc', padding: '1rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Pet Option</span>
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{selectedPetOption.label}</span>
              </div>
